...
print_path_info(struct fileinfo finfo)
{
  ...
  if (opts.user) {
    /*  Hint: getpwuid(3) */
    struct passwd *user_info = getpwuid(finfo.st.st_uid);
    char *username = strdup(user_info ->pw_name);
    if (printf("%c%s", sep, username) < 0) goto exit; /* TODO */
    sep = ' ';
    free(username);
  }
  if (opts.group) {
    /*  Hint: getgrgid(3) */
    struct group *group_info = getgrgid(finfo.st.st_gid);
    if (printf("%c%s", sep, group_info -> gr_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.size) {
    /*  Hint: stat.h(0p) */
